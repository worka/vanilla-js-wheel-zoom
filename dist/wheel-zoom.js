(function (global, factory) {
  ...
  function DragScrollable(windowObject, contentObject) {
    ...
    this.options = extendObject({
      ...
      onDrop: null
    }, options); // check if we're using a touch screen
    ...
    if (this.content.$element) {
      ...
      if (this.options.type === 'image') {
        ...
        if (!initAlreadyDone || this.options.watchImageChange === true) {
          ...
          on(this.content.$element, 'load', this._init, // if watchImageChange == false listen add only until the first call
          this.options.watchImageChange ? false : {
            once: true
          });
        }
      } else {
  ...
  WZoom.prototype = {
    ...
    _prepare: function _prepare() {
      ...
      this.content.$element.style.transform = "translate3d(".concat(this.content.alignPointX, "px, ").concat(this.content.alignPointY, "px, 0px) scale(").concat(this.content.minScale, ")");
    ...
    _computeNewPosition: function _computeNewPosition(contentNewScale, _ref) {
      ...
      var _document = document,
          ...
          documentElement = _document.documentElement;
      var scrollLeft = window.pageXOffset || documentElement.scrollLeft || body.scrollLeft;
      var scrollTop = window.pageYOffset || documentElement.scrollTop || body.scrollTop; // calculate the parameters along the X axis

      var leftWindowShiftX = x + scrollLeft - window.positionLeft;
      var centerWindowShiftX = window.originalWidth / 2 - leftWindowShiftX;
      var centerContentShiftX = centerWindowShiftX + content.currentLeft;
      var contentNewLeft = centerContentShiftX * (contentNewWidth / content.currentWidth) - centerContentShiftX + content.currentLeft; // check that the content does not go beyond the X axis
      ...
      var centerWindowShiftY = window.originalHeight / 2 - topWindowShiftY;
      var centerContentShiftY = centerWindowShiftY + content.currentTop;
      var contentNewTop = centerContentShiftY * (contentNewHeight / content.currentHeight) - centerContentShiftY + content.currentTop; // check that the content does not go beyond the Y axis
      ...
      switch (this.options.alignContent) {
        ...
        default:
          if ((contentNewHeight - window.originalHeight) / 2 + content.correctY < Math.abs(contentNewTop)) {
            ...
            contentNewTop = ((contentNewHeight - window.originalHeight) / 2 + content.correctY) * _positive;
          }
    ...
    _zoom: function _zoom(direction) {
      ...
      this._transform(this._computeNewPosition(this._computeNewScale(direction), {
  ...
  function _wheelHandler(event) {
    if (!this.options.disableWheelZoom) {
      ...
      this._transform(this._computeNewPosition(this._computeNewScale(event.deltaY), {
  ...
  function _upHandler(event) {
    if (this.coordsOnMouseDown && this.coordsOnMouseDown.x === eventClientX(event) && this.coordsOnMouseDown.y === eventClientY(event)) {
      this._transform(this._computeNewPosition(this.direction === 1 ? this.content.maxScale : this.content.minScale, {
        x: eventClientX(event),
        y: eventClientY(event)
      }));
  ...
  function _calculateCorrectPoint(options, content, window) {
    ...
    if (options.alignContent === 'left') correctX = correctX * 2;else if (options.alignContent === 'right') correctX = 0;
    if (options.alignContent === 'bottom') correctY = correctY * 2;else if (options.alignContent === 'top') correctY = 0;
