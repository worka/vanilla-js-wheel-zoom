(function (global, factory) {
    ...
    function JcWheelZoom(selector) {
      ...
      this.isTouch = 'ontouchstart' in window || navigator.MaxTouchPoints > 0 || navigator.msMaxTouchPoints > 0; // switch to touch events if using a touch screen
    ...
    JcWheelZoom.prototype = {
      ...
      _init: function _init() {
        ...
        this.content.maxScale = round(this.content.$element.naturalWidth / this.content.$element.offsetWidth * this.options.maxScale); // initial `image` sizes

        this.content.initialWidth = this.content.$element.offsetWidth;
        this.content.initialHeight = this.content.$element.offsetHeight; // current `image` sizes and transform data
        ...
        on(this.window.$element, 'wheel', function (event) {
          ...
          _this._transform(_this._computeNewPosition(_this._computeNewScale(event.deltaY), {
        ...
        on(self, 'resize', function (event) {
          ...
          _this._transform(_this._computeNewPosition(1, {
            x: eventClientX(event),
            y: eventClientY(event)
          }));
        ...
        on(this.window.$element, this.events.down, function () {
          clickExpired = false;
          setTimeout(function () {
            return clickExpired = true;
          }, 150);
        }, this.events.options);
        on(this.window.$element, this.events.up, function (event) {
          if (!clickExpired) {
            _this._transform(_this._computeNewPosition(_this.direction === 1 ? _this.content.maxScale : 1, {
              x: eventClientX(event),
              y: eventClientY(event)
            }));
      ...
      _transform: function _transform(_ref2) {
        ...
        this.content.$element.style.transform = newScale === 1 ? null : "translate3d(".concat(newLeft, "px, ").concat(newTop, "px, 0px) scale(").concat(newScale, ")");
      },
