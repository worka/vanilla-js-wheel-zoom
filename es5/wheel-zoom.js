...
(function JcWheelZoomModule(factory) {
  ...
  JcWheelZoom.prototype = {
    ...
    _prepare: function _prepare() {
      ...
      var minScale = Math.min(this.original.window.width / this.original.image.width, this.original.window.height / this.original.image.height); // calculate margin-left and margin-top to center the image

      this.correctX = Math.max(0, (this.original.window.width - this.original.image.width * minScale) / 2);
      this.correctY = Math.max(0, (this.original.window.height - this.original.image.height * minScale) / 2); // set new image dimensions to fit it into the container
      ...
      this.container.style.width = "".concat(this.image.width + this.correctX * 2, "px");
      this.container.style.height = "".concat(this.image.height + this.correctY * 2, "px");
    ...
    _rescale: function _rescale(event) {
      ...
      var minScale = Math.min(this.original.window.width / this.original.image.width, this.original.window.height / this.original.image.height); // new allowed proportion of scale

      var newScale = scale + delta / this.options.speed;
      newScale = newScale < minScale ? minScale : newScale > this.options.maxScale ? this.options.maxScale : newScale; // scroll along the X axis before resizing
